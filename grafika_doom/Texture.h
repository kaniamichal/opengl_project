...
public:
	...
	Texture(const char* fileName, GLenum type, GLint texture_unit)
	{
		...
		{
			std::cout << "ERROR, I can't load file with texture" << fileName << "\n";
		}
	...
	void loadFromFile(const char* fileName)
	{
		...
		glTextureParameteri(this->type, GL_TEXTURE_WRAP_S, GL_REPEAT);
		glTextureParameteri(this->type, GL_TEXTURE_WRAP_T, GL_REPEAT);
		glTextureParameteri(this->type, GL_TEXTURE_MAG_FILTER, GL_LINEAR_MIPMAP_LINEAR);
		glTextureParameteri(this->type, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
